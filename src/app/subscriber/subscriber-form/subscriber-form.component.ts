...
export class SubscriberFormComponent implements OnInit, OnDestroy {
  ...
  pCommodities$: Observable<Lookup[]>
  ...
  private loadSubscriberTypeCommodities(subscriberTypeId: number) {
    ...
    this.commodities$ = this.subscriberService.fetchCommoditiesBySubscriberType(subscriberTypeId, false)
      .pipe(
        shareReplay(1),
  ...
  private loadSubscriberTypePrimaryCommodities(subscriberTypeId: number) {
    this.loadingCommodities = true
    this.pCommodities$ = this.subscriberService.fetchCommoditiesBySubscriberType(subscriberTypeId, true)
      .pipe(
        shareReplay(1),
        finalize(() => this.loadingCommodities = false)
      )
  ...
  private subscriberTypeValueChangeListener() {
    this.subscriberTypeId.valueChanges
      ...
      .subscribe(value => {
        ...
        this.loadSubscriberTypePrimaryCommodities(value)
