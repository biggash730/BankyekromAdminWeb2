...
export class SubscriberService {
  ...
  fetchCommoditiesBySubscriberType(subscriberTypeId: number, isPrimary: boolean) {
    return this.http.get<ResponseObject<Lookup[]>>(`${environment.baseUrl}/commodity/getcommodities?subscriberTypeId=${subscriberTypeId}&&isPrimary=${isPrimary}`)
      .pipe(
        map(res => {
          if (res.success) { return res.data }
        })
